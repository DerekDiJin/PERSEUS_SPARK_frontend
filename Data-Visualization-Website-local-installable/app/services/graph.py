...
class Graph:
    ...
    def graph_from_file(self, dataset, cache=False):
        ...
        print "????: " + str(dataset.analyzed_graph_file)

        dataset_filename = absolute_path(str(dataset.analyzed_graph_file))
        print dataset_filename
        data = pd.read_csv(dataset_filename, skipinitialspace=True, escapechar="\\", header=None, encoding='utf8', engine='python')
        data_string = pd.read_csv(dataset_filename, skipinitialspace=True, escapechar="\\", header=None, dtype=str, encoding='utf8', engine='python')
